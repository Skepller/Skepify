...
Vectorizer.prototype.loadPixels = function(callback) {
    ...
    if (error) {
        ...
        console.log('Response: Body: Length: %d. Is buffer: %s', body.length, (body.buffer instanceof ArrayBuffer));
				console.log(body);

				try {
				var rawImageData = decode(body.buffer);
				...
				try {
					var t = APNG.parseBuffer(body.buffer);
	...
	restler.get(this.url, { decoding: 'buffer' }).on('complete', function(result) {
	  if (result instanceof Error) {
		...
		callback(false);
	  	...
	  	try {
			var rawImageData = decode(result);
			_this.width = rawImageData.width;
			_this.height = rawImageData.height;
			_this.pixels = rawImageData.data;
			callback(true);
